...
export const AlumniSearch: React.FC<AlumniSearchProps> = ({
  ...
  const handleResultSelect = useCallback((result: SearchResult) => {
    // Only show alumni results in this component
  ...
  if (!isExpanded) {
    return (
      <Card className={`alumni-search cursor-pointer hover:shadow-sm transition-shadow ${className}`}>
        <CardContent 
          className="p-4"
        ...
        >
          <div className="flex items-center space-x-3">
            ...
            <span className="font-medium">Search Alumni Records</span>
          </div>
        </CardContent>
      </Card>
    );
  ...
  // Expanded state
  return (
    <Card className={`alumni-search ${className}`}>
