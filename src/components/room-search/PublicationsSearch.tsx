// Publications Room Search Component
...
export const PublicationsSearch: React.FC<PublicationsSearchProps> = ({
  onResultSelect,
  initialQuery,
  className = ""
}) => {
  ...
  const handleResultSelect = useCallback((result: SearchResult) => {
    // Only show publication results in this component
  ...
  if (!isExpanded) {
    return (
      <Card className={`publications-search cursor-pointer hover:shadow-sm transition-shadow ${className}`}>
        <CardContent 
          className="p-4"
        ...
        >
          <div className="flex items-center space-x-3">
            ...
            <span className="font-medium">Search Publications</span>
          </div>
        </CardContent>
      </Card>
    );
  ...
  // Expanded state
  return (
    <Card className={`publications-search ${className}`}>
